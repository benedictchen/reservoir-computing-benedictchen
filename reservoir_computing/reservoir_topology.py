...
class ReservoirTopology:
    ...
    def create_small_world(self, size: int = None, connectivity: float = 0.1) -> np.ndarray:
        """Alias for create_small_world_topology for test compatibility"""
        k = max(1, int(connectivity * (size or self.reservoir_size)))
        return self.create_small_world_topology(k=k, p=0.3)