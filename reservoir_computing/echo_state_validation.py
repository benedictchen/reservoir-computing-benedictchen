...
class EchoStatePropertyValidator:
    ...
    def __init__(self, esn_instance=None):
        self.esn = esn_instance
    
    def validate(self, esn=None):
        """Simple validation method that returns a boolean"""
        target_esn = esn or self.esn
        if target_esn is None:
            return False
        
        # Use the existing comprehensive validation but return just the boolean
        try:
            # Quick spectral radius check
            eigenvals = np.linalg.eigvals(target_esn.W_reservoir)
            spectral_radius = np.max(np.abs(eigenvals))
            return bool(spectral_radius < 1.0)
        except:
            # Fallback: assume valid if no reservoir weights available
            return True
